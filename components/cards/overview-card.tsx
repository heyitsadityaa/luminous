...
export function OverviewCard({ className }: { className?: string }) {
    ...
    const [overviewData, setOverviewData] = useState<OverviewData>(INITIAL_OVERVIEW_DATA)
    const [draftData, setDraftData] = useState<OverviewData>(INITIAL_OVERVIEW_DATA)
    ...
    const savingsProgress = Math.round((overviewData.currentSavings / overviewData.savingsGoal) * 100)
    const creditUsed = Math.round((overviewData.creditOutstanding / overviewData.creditLimit) * 100)
    ...
    const viewTotalInvested = isEditing
        ...
        : totalInvested
    const viewSavingsProgress = Math.round((viewData.currentSavings / viewData.savingsGoal) * 100)
    const viewCreditUsed = Math.round((viewData.creditOutstanding / viewData.creditLimit) * 100)

    return (
        <Card className={cn("dark:bg-primary/80 dark:text-black bg-foreground text-background", className)}>
            <CardHeader className="pb-2 flex flex-row items-start justify-between">
                <div>
                    <span className="text-xs font-medium uppercase tracking-widest opacity-60">Total Balance</span>
                    <div className="flex items-end gap-1">
                        {isEditing ? (
                            <Input
                                value={String(viewData.totalBalance)}
                                onChange={(e) => updateDraftNumber("totalBalance", e.target.value)}
                        ...
                        <span className="font-mono text-lg font-semibold mb-0.5 opacity-70">₹</span>
                    </div>
                </div>
            ...
            <CardContent className="space-y-5">
                    ...
                    <div className="flex flex-col gap-0.5">
                        ...
                        {isEditing ? (
                            <Input
                                ...
                                onChange={(e) => updateDraftNumber("totalIncome", e.target.value)}
                                className="h-8 bg-transparent border-white/30 text-white dark:text-black"
                            />
                        ) : (
                            <span className="font-mono font-semibold text-sm">{formatINR(viewData.totalIncome)}</span>
                        )}
                    </div>
                    <div className="flex flex-col gap-0.5">
                        ...
                        {isEditing ? (
                            <Input
                                ...
                                onChange={(e) => updateDraftNumber("totalExpense", e.target.value)}
                                className="h-8 bg-transparent border-white/30 text-white dark:text-black"
                            />
                        ) : (
                            <span className="font-mono font-semibold text-sm">{formatINR(viewData.totalExpense)}</span>
                        )}
                    </div>
                    <div className="flex flex-col gap-0.5">
                ...
                <div className="space-y-2">
                        ...
                        {isEditing ? (
                            <div className="flex items-center gap-2">
                                <Input
                                    ...
                                    onChange={(e) => updateDraftNumber("currentSavings", e.target.value)}
                                    className="h-8 w-24 bg-transparent border-white/30 text-white dark:text-black"
                                />
                                <span>/</span>
                                <Input
                                    value={String(viewData.savingsGoal)}
                                    onChange={(e) => updateDraftNumber("savingsGoal", e.target.value)}
                                    className="h-8 w-24 bg-transparent border-white/30 text-white dark:text-black"
                                />
                            </div>
                        ) : (
                            <span className="font-mono text-xs opacity-70">
                                {formatINR(viewData.currentSavings)} / {formatINR(viewData.savingsGoal)}
                            </span>
                        )}
                    </div>
                    <Progress
                    ...
                    />
                    <span className="text-xs opacity-50">{viewSavingsProgress}% of annual goal reached</span>
                </div>
                ...
                <div className="relative rounded-2xl p-4 overflow-hidden dark:bg-black/20 bg-white/10 border dark:border-black/10 border-white/10">
                    ...
                    <div className="relative z-10 flex flex-col gap-4">
                        <div className="flex items-center justify-between">
                            <span className="text-xs font-semibold uppercase tracking-widest opacity-70">State Bank of India</span>
                        ...
                        <div className="flex items-end justify-between">
                            <div>
                                <div className="font-mono text-xs opacity-60 mb-0.5 tracking-widest">•••• •••• •••• 4455</div>
                ...
                <div className="space-y-2">
                    <div className="flex items-center justify-between">
                            ...
                            <Badge className="text-xs px-1.5 py-0 dark:bg-black/20 bg-white/20 border-0 dark:text-black text-background">
                                ...
                                {isEditing ? (
                                    <Input
                                        ...
                                        onChange={(e) => updateDraftString("creditDueDate", e.target.value)}
                        ...
                        {isEditing ? (
                            <Input
                                ...
                                onChange={(e) => updateDraftNumber("creditOutstanding", e.target.value)}
                                className="h-8 w-24 bg-transparent border-white/30 text-white dark:text-black"
                            />
                        ) : (
                            <span className="font-mono text-xs font-semibold">{formatINR(viewData.creditOutstanding)}</span>
                        )}
                    </div>
                    <Progress
                    ...
                    <div className="flex justify-between">
                        <span className="text-xs opacity-50">{viewCreditUsed}% of limit used</span>
                        {isEditing ? (
                            <Input
                                value={String(viewData.creditLimit)}
                                onChange={(e) => updateDraftNumber("creditLimit", e.target.value)}
                                className="h-8 w-28 bg-transparent border-white/30 text-white dark:text-black"
                            />
                        ) : (
                            <span className="text-xs opacity-50">Limit {formatINR(viewData.creditLimit)}</span>
                        )}
                    </div>
                </div>
                ...
                <div className="space-y-3">
                    <div className="flex items-center justify-between">
                        <span className="text-xs font-semibold uppercase tracking-widest opacity-60">Investments</span>
                        <span className="font-mono text-xs font-bold">{formatINR(viewTotalInvested)} total</span>
                    </div>
                    {viewData.investments.map((inv) => (
                        ...
                        >
                            ...
                            <div className="flex flex-col items-end gap-0.5">
                                <span className="font-mono text-sm font-bold">{formatINR(inv.amount)}</span>
                            </div>
                        </div>
                    ))}
                </div>
                ...
                <div className="space-y-2">
                    <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-xs font-bold uppercase tracking-widest opacity-60">Alerts</span>
                    </div>
